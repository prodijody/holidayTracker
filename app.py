...
def index():
  ...
  if request.method == 'POST' and loginForm.validate_on_submit():
    ...
    if user := User.query.filter_by(email=escaped_email).first():
...
def menu():
  if current_user.is_admin():
    ...
    if pending_requests > 0:
      flash(
          f"""You have <strong>{pending_requests}</strong> holiday request(s) waiting for un update. <a href="{url_for('admin')}">Click here</a>.""",
          'info',
      )
...
def calendar():
  ...
  requests_for_calendar = [{
      'title': f'{i.get_user().name} {i.get_user().surname}',
      'start': str(i.date_from),
      'end': str(i.date_to),
      'url': url_for('open_request', request_id=i.id),
  } for i in requests]
...
def open_request(request_id):
  ...
  if request_exists := HolidayRequest.query.get(req):
    ...
    flash(f'That request id {request_id} is invalid.', 'danger')
    return redirect(url_for('calendar'))
...
def admin_user(user_id):
  ...
  if user := User.query.get(user_id):
    ...
    form2.system_role.choices = [
        (i.id, f'{str(i.id)}) {i.name}')
        for i in SystemRole.query.order_by(SystemRole.id.desc()).all()
    ]
...
def add_user():
  ...
  addUserForm.system_role.choices = [
      (i.id, f'{str(i.id)}) {i.name}')
      for i in SystemRole.query.order_by(SystemRole.id.desc()).all()
  ]
